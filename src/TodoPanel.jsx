    ...
    render() {
        ...
        var snip = this.props.todoList.map((element, index, array) => {
            ...
            if (this.props.todoList[index].priority == 'High') {
                ...
                optionSelected = 'Low';
            } else liClasses = 'form-group panel-body bg-success'
            ...
            if(!this.props.todoList[index].editEnabled) {
                return (
                    <li key={index} style={marginBottomZero} className={liClasses}>
                        ...
                        <div className='pull-right form-group' style={inline}>
                            <a style={marginRight} id='edit-link' href='#' onClick={() => this.editTodo(index)}><i className="edit-todo glyphicon glyphicon-edit"></i></a>
                            <a id='delete-link' href='#' onClick={() => this.deleteTodo(index)}><i className="delete-todo glyphicon glyphicon-trash"></i></a>  
                        </div>
                    </li>
                )
            } else {
                return (
                    <li key={index} style={marginBottomZero} className={liClasses}>
                        ...
                        <textarea onChange={this.updateDescription} type='text' id='edit-description' defaultValue={this.state.editing.desc} className='update-todo-text form-control'/>
                        <div className='row'>
                            <div style={margin} className='col-md-6'>
                                <label>Due Date</label>
                                <input type='text' onChange={this.updateDate} defaultValue={this.state.editing.dueDate} id='edit-date' className='update-todo-date form-control'/>
                            </div>
                            <div style={margin} className='col-md-6'>
                                <label>Priority</label>
                                <select onChange={this.updatePriority} className='update-todo-priority btn-block' defaultValue={optionSelected}>
                        ...
                        <div className='text-right form-group'>
                            <button style={margin} className='update-todo btn btn-success' onClick={() => this.saveTodo(index, listItem)}>Save</button>
                        </div>
                    </li>
                )
            }
